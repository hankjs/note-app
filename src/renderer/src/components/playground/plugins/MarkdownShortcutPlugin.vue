...
import { LexicalMarkdownShortcutPlugin, HR } from "lexical-vue"
...
import { CodeNode, $isCodeNode, $createCodeNode } from "../nodes/CodeNode"
...
const CODE: Transformer = {
  ...
  replace: (rootNode, children, startMatch, endMatch, linesInBetween, isImport) => {
    console.log("replace rootNode: ", rootNode)
    console.log("replace children: ", children)
    console.log("replace startMatch: ", startMatch)
    console.log("replace endMatch: ", endMatch)
    console.log("replace linesInBetween: ", linesInBetween)
    console.log("replace isImport: ", isImport)
    let codeBlockNode
    let code$1
    if (!children && linesInBetween) {
      if (linesInBetween.length === 1) {
        ...
        if (endMatch) {
          ...
          code$1 = linesInBetween[0].startsWith(" ")
...
const PLAYGROUND_TRANSFORMERS: Transformer[] = [CODE, ...TRANSFORMERS]
</script>
