...
const emit = defineEmits<{
  (e: "change", value: string, skipHistoryStack: boolean, skipRefocus: boolean): void
}>()

const skipAddingToHistoryStack = ref(false)
// 是否监听了 change 事件
const onChangeListener = ref(false)
...
const emitOnChange = (newColor: string, skipRefocus: boolean = false) => {
  ...
  if (innerDivRef.value !== null && onChangeListener.value) {
    emit("change", newColor, skipAddingToHistoryStack.value, skipRefocus)
  }
}
...
watchEffect(() => {
  ...
  if (innerDivRef.value !== null) {
    emitOnChange(selfColor.value.hex, skipAddingToHistoryStack.value)
...
onMounted(() => {
  ...
  onChangeListener.value = typeof props.onChange === "function"
})
</script>
  ...
  >
    ...
    <div class="color-picker-basic-color">
      <button
        ...
        @click="onBasicColorClick($event, basicColor)"
      />
    </div>
    <MoveWrapper
